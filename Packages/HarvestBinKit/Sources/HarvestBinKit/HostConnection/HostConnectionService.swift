...
public protocol HostConnectionService: Sendable {
  ...
  func setCommandHandler(_ handler: @escaping @Sendable (HarvestCommand) async throws -> HarvestResponse)
}
...
public final class DefaultHostConnectionService: HostConnectionService {
  ...
  public func setCommandHandler(_ handler: @escaping @Sendable (HarvestCommand) async throws -> HarvestResponse) {
    ...
    do {
      ...
      if let handler = commandHandler {
        ...
        response = HarvestResponse(
          ...
          timestamp: Date(),
          status: .error,
          payload: .error(HarvestErrorResponse(
            code: "NO_HANDLER",
