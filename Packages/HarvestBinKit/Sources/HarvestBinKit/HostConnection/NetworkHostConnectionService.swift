//
// NetworkHostConnectionService.swift
// HarvestBinKit
//
...
import Foundation
  ...
  private final class HostConnection: Hashable, Sendable {
    ...
    private func receiveData() async throws -> Data {
      return try await withCheckedThrowingContinuation { continuation in
        nwConnection.receive(minimumIncompleteLength: 1, maximumLength: 65536) { data, _, _, error in
          if let error = error {
...
#if !canImport(Network) && !canImport(Virtualization)
  /// Stub implementation for platforms without Network or Virtualization
